...
class TreeTaggerWriter
  ...
  def create_files(opts = {})
    ...
    descr = opts[:descr] || create_descr(base_name)

    # Initialize lexicon and open class file lists for each fold
    # TODO hacky, must be a better way to create array of arrays/hashes
    lexicon = (1..@reader.num_folds).collect { |i| Hash.new }
    open_classes = (1..@reader.num_folds).collect { |i| Array.new }
    ...
    descr = descr[0] unless @reader.has_folds?
  ...
  def create_descr(base_name=nil)
    if base_name
      if @reader.has_folds?
        (0...@reader.num_folds).collect do |i|
        ...
        { in_file: File.new("#{base_name}_in", 'w'),
